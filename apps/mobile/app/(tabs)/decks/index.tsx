...
export default function DecksScreen() {
  ...
  // Set header right button on mobile (native Stack header)
  useLayoutEffect(() => {
    if (isDesktop) return;
        ...
        setDecks((prev) =>
          prev.map((deck) =>
            deck.archidektId === archidektId
              ? {
                  ...deck,
                  syncStatus: "waiting" as DeckSyncStatus,
                  syncError: null,
                }
              : deck,
          ),
  ...
  const renderEmptyState = () => (
    <View className="flex-1 items-center justify-center">
      ...
      <Button
        ...
        variant={archidektConnected ? "secondary" : "default"}
      >
        <View className="flex-row items-center gap-2">
          <Plus
            size={16}
            color={
              archidektConnected ? (isDark ? "#e2e8f0" : "#475569") : "white"
            }
          />
          <Text
            className={`font-medium ${archidektConnected ? (isDark ? "text-slate-200" : "text-slate-700") : "text-white"}`}
          >
            Create New Deck
          </Text>
        </View>
  ...
  return (
    ...
    >
      ...
      {dropdownVisible && (isDesktop ? archidektConnected : true) && (
        <View
          className={`absolute right-4 lg:right-6 ${isDesktop ? "top-20" : "top-2"} z-50 w-56 rounded-lg border shadow-lg ${
            isDark
        ...
        {isDesktop && (
          <View className="flex-row items-center justify-between px-6 py-4">
            ...
            <SplitHeaderButton
              ...
              showSecondary={archidektConnected}
            />
          </View>
        )}
